...
import static sn.atos.wordline.nsbf.filetranfertgrpc.srv.FileTransfertManager.Serializers.*;
  ...
  public class FileTransfertManagerHandlerFactory {
     ...
     *
     * Use `akka.grpc.scaladsl.ServiceHandler.concatOrNotFound` with `FileTransfertManagerHandler.partial` when combining
     * several services.
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> create(FileTransfertManager implementation, Materializer mat, ActorSystem system) {
      return create(implementation, FileTransfertManager.name, mat, system);
    }
     ...
     * several services.
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> create(FileTransfertManager implementation, Materializer mat, Function<ActorSystem, Function<Throwable, io.grpc.Status>> eHandler, ActorSystem system) {
      return create(implementation, FileTransfertManager.name, mat, eHandler, system);
    }
     ...
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> create(FileTransfertManager implementation, String prefix, Materializer mat, ActorSystem system) {
     ...
     * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> create(FileTransfertManager implementation, String prefix, Materializer mat, Function<ActorSystem, Function<Throwable, io.grpc.Status>> eHandler, ActorSystem system) {
     ...
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> partial(FileTransfertManager implementation, String prefix, Materializer mat, ActorSystem system) {
      return partial(implementation, prefix, mat, GrpcExceptionHandler.defaultMapper(), system);
    }
     ...
     * Use `akka.grpc.javadsl.ServiceHandler.concatOrNotFound` when combining several services.
     */
    public static Function<HttpRequest, CompletionStage<HttpResponse>> partial(FileTransfertManager implementation, String prefix, Materializer mat, Function<ActorSystem, Function<Throwable, io.grpc.Status>> eHandler, ActorSystem system) {
      return (req -> {
      ...
      public String getServiceName() {
        return FileTransfertManager.name;
      }

    private static CompletionStage<HttpResponse> handle(HttpRequest request, String method, FileTransfertManager implementation, Materializer mat, Function<ActorSystem, Function<Throwable, io.grpc.Status>> eHandler, ActorSystem system) {
      Codec responseCodec = Codecs.negotiate(request);
      
      switch(method) {
        
        case "PostFile":
          return GrpcMarshalling.unmarshal(request, filePartSerializer, mat)
